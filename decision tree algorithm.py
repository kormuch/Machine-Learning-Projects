...
def classify(row, node):
    print(f"classify executed for {row}, {node}")
    if node['type'] == 'leaf':
        print("    nodetype = leaf, returning predictions")
        print(f"    node['predictions'] = {node['predictions']}")
        return node['predictions']
    if match(row, node['column'], node['value']):
        print(f"   match for {node['value']}")
        print(f"   classifying {row}  as true_branch")
        return classify(row, node['true_branch'])
    else:
        print(f"   row {row} belongs to false_branch")
